...
function order() {
  inquirer.prompt([{
    ...
    .then(function (answer) {
      connection.query("SELECT stock_quantity FROM saleItems WHERE item_id = ? ",
        [answer.item_id],
        function (err, res) {
          if (err) {
            console.log(err)
...
function displayPrice(id, quantity) {
    connection.query("SELECT price FROM saleItems WHERE item_id = ? ",
      [id],
      function (err, res) {
        if (err) {
          console.log(err)
...
function showStuff() {
    connection.query("SELECT item_id, product_name, price FROM saleItems",
      function (err, res) {
        if (err) {
          console.log(err)
        };
